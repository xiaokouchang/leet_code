...
1. 排序
2. 固定一个数，如果固定的数大于0，后面的数都是大于0，所有数之和一定大于0
3. 在该数后面的区间内，利用”双指针算法“快速找到两个和等于-a即可

**细节问题**

1. 去重（越界访问）

   找到一种结果后，left和right要跳过重复元素
...
1. 依次固定一个数a
2. 在a后面的区间内，利用”三数之和“（依次固定一个数b，在b后面的区间内，利用”双指针“找到2个数，使这2个数的和等于target-a-b即可）找到这3个数，使这3个数的和等于target-a即可。
